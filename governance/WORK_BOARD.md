...
- **Network prerequisite (Stage C blocker):** [`governance/ENVIRONMENT_NETWORK_REQUIREMENTS.md`](ENVIRONMENT_NETWORK_REQUIREMENTS.md) — environment is on the "Trusted" network level, which blocks all HIS-001 sources. The User must add the listed domains to the environment allowlist before source verification (and therefore Stage C evidence-ledger construction) can proceed. Re-test and upgrade Source Register access statuses once configured.
