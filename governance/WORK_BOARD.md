...
- **Next review:** Gate 1 re-review — confirmation that the 8 required revisions are correctly applied (verdict must move PASS WITH CONDITIONS → confirmed PASSED). Broad evidence collection begins only after the Gate 1 re-review is confirmed PASSED (the Execution Plan's internal check is done).
